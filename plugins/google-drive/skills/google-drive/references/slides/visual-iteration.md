...
Use [visual-change-loop](./visual-change-loop.md) as the compact reusable version of this workflow when another Slides task changes visible layout or styling.
...
- Escalate to [google-slides-template-surgery](./template-surgery.md) when a slide still has structural layout problems after 3-4 verified passes, or when the same issue repeats across multiple slides.
