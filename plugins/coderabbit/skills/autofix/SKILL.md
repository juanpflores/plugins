...
### Required Tools
- `coderabbit` (CodeRabbit CLI) - see install steps below
...
If not installed, STOP the skill and tell the user:

```
CodeRabbit CLI is not installed. Please install it before running autofix.
...
Do NOT continue with the workflow. Wait for the user to install the CLI and re-invoke the skill.
